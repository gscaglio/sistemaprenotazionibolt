...
import { addMonths, subMonths, format, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, isToday, isTomorrow, isWithinInterval, isBefore, isAfter } from 'date-fns';
...
export function Calendar({ mode = 'single', selectedDates = [], onSelect, className, currentRoomId }: CalendarProps) {
  ...
  const handleBulkAvailabilityUpdate = async (available: boolean) => {
    ...
    try {
      ...
      console.error('Error during bulk availability update:', error);
    } finally {
      setIsBulkUpdating(false);
    }
  };
  ...
  const handleRevertToBasePrice = async () => {
    ...
    try {
      ...
      console.error('Error during revert to base price:', error);
    } finally {
      setIsBulkUpdating(false);
    }
  };
